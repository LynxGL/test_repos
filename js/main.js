...
function timeAgo(dateString) {
    ...
    if (secondsPast < 60) {
        return parseInt(secondsPast) + ' секунд назад';
    }
    if (secondsPast < 3600) {
        return parseInt(secondsPast/60) + ' минут назад';
    }
    if (secondsPast <= 86400) {
        return parseInt(secondsPast/3600) + ' часов назад';
    }
    if (secondsPast > 86400) {
...
function createProject(demoProject) {
  ...
  if (demoProject.length <= 0 || demoProject === "undefined") {
    ...
    demoProject.map((data, index) => {
      ...
      div.innerHTML = `
            <div class="project-box" style="background-color: ${randomHSLA(
              0.75
            )}">
